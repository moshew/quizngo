...
גרסה: 1.1 (שלב א' — עורך בלבד; 1.1 = פישוט העורך + תבניות מעוצבות, ראו סעיף 16)
תיקייה: `app/`
...
Quiz {
  ...
  slides: Slide[]
  schemaVersion: 2                   // 1→2: החלה מחדש של התבנית (skins); ראו FR-18
}
```
...
Slide {
  ...
  background: { kind: "color" | "gradient" | "image", color?, gradient?: {angle, stops[]}, src?, overlay?, decor?: boolean }   // decor=false מכבה את עיטורי התבנית
  layout?: "text" | "banner" | "side" | "image-answers"   // רק ל-type === "question" (FR-19)
...
### FR-03: תבניות
- 6 תבניות מובנות בשתי "משפחות עיצוב" (skins). שתי התבניות הראשיות הן העיצובים שאושרו (Claude Design) והשאר נגזרות מהן:
  - משפחת **chunky** (מילוי שטוח, גבול ink עבה, צל תלת-ממדי קשיח, רדיוסים גדולים): `magenta-party` (העיצוב המקורי; זהה גם ל-`game/new_design/quizngov.pptx`), `sunset`, `ocean`, `minimal-light` (גרסת יום בהירה).
  - משפחת **neon** (פאנלי זכוכית, קצוות ניאון, פינות חדות, מטא-דאטה במונוספייס, מסגרת HUD): `midnight-arcade` (העיצוב המקורי), `classic-black` (שחור-זהב).
- תבנית = `skin` + טוקנים (צבעים/גופנים כ-CSS variables) + רקע + `decor` (שכבת עיטור CSS שאינה אלמנט) + **layout לכל סוג שקף** (פונקציה שמייצרת אלמנטים ב-1920×1080; הגיאומטריה נגזרת מהעיצובים ומה-PPTX).
- המראה של תשובות ו-widgets מגיע מה-skin (CSS). שדות סגנון באלמנט (`background`, `border`, `shadow`, `borderRadius`, `color`, `fontFamily`) הם **override** בלבד: `null` = "לפי התבנית".
- חידון חדש מתבנית מכיל: פתיחה, שאלה לדוגמה, התפלגות, מובילים, סיכום.
- הוספת שקף חדש משתמשת ב-layout של התבנית הנוכחית של החידון.
- "החל תבנית" על חידון קיים: מחליף רקעים וסגנון של אלמנטים בעלי `binding`/`answer`/`widget` ושומר את תוכן השאלות ואת פריסת השאלה (`slide.layout`); אלמנטים חופשיים נשמרים כפי שהם (עם אישור המשתמש). זמין ישירות מהסרגל העליון ("תבנית") וגם מההגדרות.
- בוחר התבניות מציג לכל תבנית שקף ראשי + 3 שקפים קטנים (שאלה/התפלגות/סיכום); במצב "החל" התצוגה מבוססת על תוכן החידון עצמו.

### FR-04: עורך — מבנה מסך
עיקרון מנחה (1.1): **תוכן קודם, עיצוב לפי דרישה**. מה שנחוץ לכתיבת חידון גלוי תמיד; כל השאר מקופל (progressive disclosure) ונזכר בין ביקורים.
- **סרגל עליון** (8 פקדים): חזרה לספרייה, שם החידון (עריכה inline) + מצב שמירה, Undo/Redo, "שאלות" (טופס bulk), "תבנית", "תצוגה מקדימה" (הפעולה הראשית), תפריט ⋯ (הגדרות החידון, קיצורי מקלדת, שפת ממשק, "הפעל משחק — בקרוב", לספרייה, התנתקות).
...
- **קנבס**: שקף במרכז עם צל, זום (−/ערך/+; לחיצה על הערך = Fit; גלגלת+Ctrl), קווי הצמדה (snap). עיטורי התבנית אינם אלמנטים ואינם ניתנים לבחירה.
- **סרגל הוספה** (4 פקדים): טקסט, תמונה, צורה (גריד צורות), רכיב דינמי (תפריט). "רקע" עבר לפאנל השקף; "החזר תשובה" מופיע בטופס השאלה רק כשכרטיס תשובה חסר.
- **פאנל ימני (Inspector)** — לפי הבחירה:
  - *שקף שאלה, ללא בחירה*: **טופס השאלה** — טקסט שאלה, 4 תשובות (טקסט/תמונה/סימון נכונה), תמונת שאלה, זמן, **בוחר פריסה** (FR-19), "אחרי השאלה" (התפלגות/מובילים). מקופלים: "רקע", "אפשרויות שקף" (סוג, הסתרה, הערות).
  - *שקף אחר, ללא בחירה*: תיאור קצר של תפקיד השקף + הגדרה רלוונטית (למשל מספר מובילים). מקופלים: "רקע", "אפשרויות שקף".
  - *אלמנט נבחר*: רק פקדי התוכן של אותו אלמנט (למשל לתשובה: טקסט, נכונה, תמונה). מקופלים: "עיצוב" (כולל "אפס לסגנון התבנית") ו"מיקום וגודל" (X/Y/W/H, סיבוב, שקיפות, שכבות, יישור). פעולות מהירות בכותרת הפאנל: שכפל, נעל, מחק.
- **סרגל טקסט צף**: גופן, גודל, B/I/U, צבע, יישור + "עוד" (רשימה, קו חוצה, הדגשה, כיוון, ניקוי). לתשובה: גודל, B, צבע בלבד.
- **תפריט הקשר** מקוצר: ערוך/חתוך/החלף · העתק/שכפל · לחזית/לרקע · נעל · מחק. שאר הפעולות נשארות בקיצורי מקלדת ובפאנל "מיקום וגודל".
- **רמז פתיחה** חד-פעמי על הקנבס (ניתן לסגירה) שמסביר את שלוש הפעולות הבסיסיות.
...
### FR-18: Skins, טוקנים ועיטורי תבנית
- שורש השקף מקבל `skin-<chunky|neon>`, `theme-<templateId>`, `type-<slideType>`, `lang-<rtl|ltr>` ואת טוקני התבנית כ-CSS variables (`--t-ink`, `--t-primary`, `--t-secondary`, `--t-danger`, `--t-accent`, `--t-text`, `--t-font-display`, `--t-font-ui`, `--t-font-mono`...). צבעי התשובות הקנוניים אינם טוקן של תבנית (ב-neon מותר גוון בהיר יותר של אותם צבעים).
- `SlideDecor` מרנדר את שכבת העיטור של התבנית מעל הרקע ומתחת לאלמנטים (כתמים, צורות רפאים, נקודות, קונפטי בסיכום / נקודות-מטריקס, רצפת גריד, כוכבים, פינות HUD). אנימציות עיטור רצות רק ב-`mode-preview`.
- כל widget ותשובה מרונדרים ב-markup סמנטי אחד; ה-skin קובע את המראה. override באלמנט (ערך שאינו `null`) גובר על ה-skin.
- טקסט שאלה ותשובות מתכווצים אוטומטית (היוריסטיקה דטרמיניסטית לפי אורך הטקסט ומידות התיבה) כך שתוכן אמיתי לא נחתך.
- מיגרציה: מסמך `schemaVersion < 2` עובר `applyTemplate(quiz, quiz.templateId)` בטעינה (עורך, Preview, ספרייה). תוכן ואלמנטים חופשיים נשמרים; אלמנטים בבעלות התבנית נבנים מחדש.

### FR-19: פריסות לשקף שאלה
- ארבע פריסות, נבחרות בלחיצה אחת מטופס השאלה: `text` (ללא תמונה, טקסט גדול), `banner` (תמונה רחבה בין השאלה לתשובות), `side` (תמונה גבוהה בצד + תשובות בטור), `image-answers` (כרטיסי תשובה עם תמונה).
- החלפת פריסה בונה מחדש רק את האלמנטים שבבעלות התבנית בשקף; התוכן והאלמנטים החופשיים נשמרים. פעולה אחת ב-Undo.
- אוטומציה: הוספת תמונת שאלה בפריסת `text` מעבירה ל-`banner`; הסרת התמונה ב-`banner`/`side` מחזירה ל-`text`; הוספת תמונה לתשובה בפריסה שאינה `image-answers` מציגה אותה כתמונה ממוזערת בצד הכרטיס.
- ברירת מחדל לשאלה חדשה: `text` (כך שלעולם אין "חור" של placeholder בזמן משחק).

### FR-20: מדדי פשטות (לבדיקת קבלה)
- שקף שאלה עם תשובה נבחרת: לא יותר מ-35 פקדים אינטראקטיביים גלויים במסך כולו (לעומת ~55 ב-1.0), ולא יותר מ-8 בפאנל הימני לפני פתיחת מקטע מקופל.
- כתיבת שאלה שלמה (טקסט, 4 תשובות, נכונה, זמן, תמונה) אפשרית מהפאנל הימני בלבד, בלי לגעת בקנבס ובלי לפתוח דיאלוג.
...
- **AC-11**: חידון חדש ב-`magenta-party` וב-`midnight-arcade` נראה כמו העיצובים שאושרו (רקע+עיטור, סרגל עליון של טיימר/מספר שאלה/עונים, כרטיסי תשובה, גרף, טבלת מובילים, פודיום) — בעברית (RTL) ובאנגלית.
- **AC-12**: החלפת פריסה של שאלה (4 פריסות) שומרת טקסט, תשובות, נכונה, תמונות ואלמנטים חופשיים, ומתבטלת ב-Undo אחד.
- **AC-13**: חידון קיים (schemaVersion 1) נפתח בעיצוב החדש של התבנית שלו בלי אובדן תוכן.
- **AC-14**: FR-20 — ספירת פקדים גלויים עומדת ביעד; כל המקטעים המקופלים נזכרים אחרי רענון.
...
### 1.1 (17 בספטמבר 2026) — פישוט העורך + תבניות מעוצבות
- **בעיה**: העורך הציג ~55 פקדים בו-זמנית (25 מהם בפאנל הימני) בלי היררכיה; התבניות היו אותה פריסה בצבעים שונים, עם מראה גנרי.
- **החלטות**: (1) progressive disclosure במקום "מצב פשוט/מתקדם" — ממשק אחד, בלי מתגים גלובליים; (2) טופס שאלה בפאנל הימני כנקודת העבודה הראשית; (3) פריסות שאלה מוכנות במקום גרירה ידנית; (4) מראה מבוסס skin (CSS) במקום העתקת ערכי סגנון לכל אלמנט — מאפשר עיצוב עשיר (זוהר, זכוכית, pseudo-elements) ונגזרות תבנית זולות; (5) עיטורים כשכבת CSS ולא כאלמנטים — פחות בחירות בטעות.
- **מקורות העיצוב**: שני ה-artifacts של Claude Design ("Magenta Party" ו-"Midnight / Neon Arcade HUD") ו-`game/new_design/quizngov.pptx` (ייצוא PowerPoint של Magenta Party, 1920×1080, עם תגי QuizNGo) ששימש לאימות הגיאומטריה.
- סעיפים שהשתנו: FR-03, FR-04, סעיף 6 (Slide.layout, background.decor, schemaVersion 2); סעיפים חדשים: FR-18, FR-19, FR-20, AC-11..14.
